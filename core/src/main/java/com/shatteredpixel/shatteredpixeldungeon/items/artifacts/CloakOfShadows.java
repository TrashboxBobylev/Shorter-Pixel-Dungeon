	...
	public class cloakStealth extends ArtifactBuff{
		...
		public boolean act(){
			...
			if (((Hero)target).hasTalent(Talent.PROTECTIVE_SHADOWS)){
				...
				if (barrier.shielding() < 1 + 3*((Hero)target).pointsInTalent(Talent.PROTECTIVE_SHADOWS)) {
			...
			if (turnsToCost <= 0){
				...
				if (charge < 0) {
					...
					//target hero level is 1 + 1.25*cloak level
					int lvlDiffFromTarget = (int) (((Hero) target).lvl - (level()*1.25f));
